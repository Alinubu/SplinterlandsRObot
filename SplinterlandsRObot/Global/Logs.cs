...
namespace SplinterlandsRObot
{
    public static class Logs
    {
        ...
        public static void OutputStat()
        {
            ...
            InstanceManager.UsersStatistics.ForEach(u => table.AddRow(u.Account, Math.Round(u.Balance.ECR,2), u.Wins, u.Draws, u.Losses, Math.Round((Convert.ToDouble(u.Wins) / ((u.Wins + u.Draws + u.Losses) == 0 ? Convert.ToDouble(1) : Convert.ToDouble((u.Wins + u.Draws + u.Losses))) * Convert.ToDouble(100)),2).ToString() + "%", u.MatchRewards, Math.Round(Convert.ToDouble(u.TotalRewards),3), u.Rating + "[" + u.RatingChange + "]", u.CollectionPower, u.Focus));
